...
header('Content-Type: text/html; charset=utf-8');
    ...
    $query = sprintf("SELECT * FROM procura ORDER BY idPer DESC");
    ...
    if(isset($_POST['buscar']))
    {
        ...
        if($filtro == 'idadeasc')
        {
            $query = sprintf("SELECT * FROM procura order by idadePer asc");
            // executa a query
            $dados = mysqli_query($con, $query) or die();
        ...
        {
            $query = sprintf("SELECT * FROM procura order by idadePer desc");
            // executa a query
            $dados = mysqli_query($con, $query) or die();
            // transforma os dados em um array
            $linha = mysqli_fetch_assoc($dados);
        ...
        {
            $query = sprintf("SELECT * FROM procura order by nomePer asc");
            // executa a query
            $dados = mysqli_query($con, $query) or die();
            // transforma os dados em um array
            $linha = mysqli_fetch_assoc($dados);
        ...
        {
            $query = sprintf("SELECT * FROM procura order by nomePer desc");
            // executa a query
            $dados = mysqli_query($con, $query) or die();
            // transforma os dados em um array
            $linha = mysqli_fetch_assoc($dados);
        ...
        {
            $query = sprintf("SELECT * FROM procura order by idPer asc");
            // executa a query
            $dados = mysqli_query($con, $query) or die();
            // transforma os dados em um array
            $linha = mysqli_fetch_assoc($dados);
...
<head>
    ...
    <title>Pets perdidos - Patinhas Felizes</title>
</head>
<body>
<header>
        <div class="nav-bar">
    ...
    <main class="lista">
        ...
        <div class="containerPet">
            <img src="<?=$linha['imagemPer']?>" alt="<?=$linha['nomePer']?>"><br>
            <div class="infos">
                <form action="../perfil_petPerdido/index.php" method="post">
                    <div class="textos">
                        <b class="acme">Nome: </b> <?=$linha['nomePer']?> <br>
                        <b class="acme">Raça: </b><?=$linha['racaPer']?><br>
                        <b class="acme">Idade: </b><?=$linha['idadePer']?>
                    </div><br>
                    <button class="acme" name="vermais" value="<?=$linha['idPer']?>"><b>Ver Mais</b></button>
                </form>
            </div>
        </div>
        <?php
    ...
    <footer class="acme branco footerLista">
